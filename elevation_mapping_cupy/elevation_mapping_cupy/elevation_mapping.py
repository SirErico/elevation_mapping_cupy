...
class ElevationMap:
    ...
    def __init__(self, param: Parameter):
        ...
        # buffers
        self.traversability_buffer = xp.full((self.cell_n, self.cell_n), 0.5)
    ...
    def update_variance(self):
        """Adds the time variance to valid cells, clamping at max_variance.

        Variance is clamped so it never exceeds max_variance.  Cells are NOT
        invalidated — they keep their last known elevation so the gridmap always
        has data around the robot.
        """
        self.elevation_map[1] += self.param.time_variance * self.elevation_map[2]
        # Clamp variance so it doesn't grow unbounded, but do NOT invalidate cells
        self.elevation_map[1] = cp.minimum(self.elevation_map[1], self.param.max_variance)
    ...
    def get_elevation(self):
        ...
        """
        return self.process_map_for_publish(self.elevation_map[0], fill_nan=False, add_z=True)
    ...
    def copy_to_cpu(self, array, data, stream=None):
        ...
        if type(array) == np.ndarray:
            ...
            if stream is not None:
                ...
                stream.synchronize()  # Ensure GPU→CPU copy completes before returning
            else:
