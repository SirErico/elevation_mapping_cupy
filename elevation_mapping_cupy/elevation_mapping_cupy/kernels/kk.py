...
def map_utils(
    ...
    util_preamble = string.Template(
        ...
        __device__ float z_noise(float16 z){
            return max(${sensor_noise_factor} * z * z, 0.01f);
        }
...
def add_points_kernel(
    ...
    add_points_kernel = cp.ElementwiseKernel(
        ...
        operation=string.Template(
            ...
            U range_sq = rx * rx + ry * ry + rz * rz;
            U v = max(${sensor_noise_factor} * range_sq, 0.01f);
                    ...
                    if (abs(map_h - z) > (map_v * ${mahalanobis_thresh})) {
                        ...
                        if (${enable_edge_shaped} && (num_points > ${wall_num_thresh}) && (z < map_h - map_v * ${mahalanobis_thresh} / num_points)) {
                          // continue;
                            ...
                            T new_v;
                            if (map_valid < 0.5f) {
                                // First measurement on empty cell — use raw z directly
                                // so the cell instantly gets the correct height
                                new_h = z;
                                new_v = v;
                            } else {
                                // Bayesian fusion with existing prior
                                new_h = (map_h * v + z * map_v) / (map_v + v);
                                new_v = max((map_v * v) / (map_v + v), 0.01f);
                            }
...
def error_counting_kernel(
    ...
    error_counting_kernel = cp.ElementwiseKernel(
        ...
        operation=string.Template(
            ...
            U y = transform_p(rx, ry, rz, R[3], R[4], R[5], t[1]);
            U z = transform_p(rx, ry, rz, R[6], R[7], R[8], t[2]);
            U range_sq_e = rx * rx + ry * ry + rz * rz;
            U v = max(${sensor_noise_factor} * range_sq_e, 0.01f);
        ...
        operation=string.Template(
            ...
            if (new_cnt > 0) {
                if (new_v / new_cnt > ${max_variance}) {
                    // Variance too high — clamp but keep last known elevation
                    map[get_map_idx(i, 1)] = ${initial_variance};
                }
                else {
                    map[get_map_idx(i, 0)] = new_h / new_cnt;
                    map[get_map_idx(i, 1)] = max(new_v / new_cnt, 0.01f);
                    map[get_map_idx(i, 2)] = 1;
                }
            }
            // Never reset cells to invalid — keep last known values
            // so the gridmap always has data around the robot
            """
