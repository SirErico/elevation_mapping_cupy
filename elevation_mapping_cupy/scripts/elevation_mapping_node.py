...
from rclpy.qos import QoSPresetProfiles, QoSProfile, QoSDurabilityPolicy, QoSReliabilityPolicy, QoSHistoryPolicy
...
class ElevationMappingNode(Node):
    ...
    def initialize_elevation_mapping(self) -> None:
        ...
        self.get_logger().info(
            f"Feature flags: visibility_cleanup={self.param.enable_visibility_cleanup}, "
            f"overlap_clearance={self.param.enable_overlap_clearance}, "
            f"drift_compensation={self.param.enable_drift_compensation}, "
            f"edge_sharpen={self.param.enable_edge_sharpen}, "
            f"time_variance={self.param.time_variance}, "
            f"max_variance={self.param.max_variance}, "
            f"initial_variance={self.param.initial_variance}"
        )
    ...
    def get_ros_params(self) -> None:
        # Hardware vs simulation flag — controls QoS profiles
        try:
            self.is_hardware = self.get_parameter('is_hardware').get_parameter_value().bool_value
        except Exception:
            self.is_hardware = False
        self.get_logger().info(f"Running in {'HARDWARE' if self.is_hardware else 'SIMULATION'} mode")
    ...
    def register_subscribers(self) -> None:
        ...
        for key, config in self.my_subscribers.items():
            ...
            if data_type == "image":
                ...
                # Use best-effort QoS for both hardware and simulation.
                # ros_gz_bridge often publishes sensor data with best-effort;
                # a reliable subscriber silently drops all messages in that case.
    ...
    def register_publishers(self) -> None:
        ...
        # QoS profile for map publishers
        if self.is_hardware:
            # Hardware: volatile + best-effort (matches sensor QoS)
            pub_qos = QoSProfile(
                depth=1,
                durability=QoSDurabilityPolicy.VOLATILE,
                reliability=QoSReliabilityPolicy.BEST_EFFORT,
            ...
            # Simulation: transient_local + reliable (late-joining rviz support)
            pub_qos = QoSProfile(
                depth=1,
        ...
        for pub_key, pub_config in self.my_publishers.items():
            ...
            publisher = self.create_publisher(GridMap, topic_name, pub_qos)
    ...
    # CHANGED bcs of namespace
    def safe_lookup_transform(self, target_frame, source_frame, time):
        # Get the node's namespace (e.g., '/j100_0000' -> 'j100_0000')
        ns = self.get_namespace().strip('/')
        
        # Try the original frame first
        frames_to_try = [source_frame]
        
        # If we have a namespace, also try with namespace prefix
        if ns and not source_frame.startswith(ns + '/'):
            frames_to_try.append(f"{ns}/{source_frame}")
        ...
        # If all failed, raise the exception with original frame name
        return self._tf_buffer.lookup_transform(
            target_frame,
            source_frame,
            time
    ...
    def image_callback(self, camera_msg: Image, camera_info_msg: CameraInfo, sub_key: str) -> None:
        ...
        try:
            ...
            self.get_logger().warn(f"Could not get transform for camera: {e}", throttle_duration_sec=5.0)
            return
    ...
    def pointcloud_callback(self, msg: PointCloud2, sub_key: str) -> None:
        ...
        try:
            # Always use latest available transform to avoid ExtrapolationException
            # from Gazebo clock jitter or delayed TF publishing.
            ...
            self.get_logger().warn(f"Could not get transform for pointcloud: {e}", throttle_duration_sec=5.0)
            return
            ...
            for channel in additional_channels:
                if hasattr(points, 'dtype') and hasattr(points.dtype, 'names') and channel in points.dtype.names:
                    ...
                    if data.ndim == 1:
                        data = data[:, np.newaxis]
                    pts = np.hstack((pts, data))
        # Filter out NaN and inf values — Gazebo gpu_lidar produces inf for
        # rays that miss all geometry, which poisons the elevation map with NaN.
    ...
    def pose_update(self) -> None:
        if self._last_t is None:
            return
        try:
            # Always use latest available transform to avoid clock jitter issues
            ...
            self.get_logger().warn(f"Could not get transform {self.map_frame} -> {self.base_frame}: {e}", throttle_duration_sec=5.0)
            return
